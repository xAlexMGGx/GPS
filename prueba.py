...
print('prim')
print([f'{key.id}: {value.id}' for key,
      value in gr.prim().items() if value is not None])
# for key,value in gr.prim().items():
...
print('dijkstra')
print([f'{key.id}: {value.id}' for key,
      value in gr.dijkstra(a).items() if value is not None])
...
new_edges = [(camino_ids[i], camino_ids[i+1])
             for i in range(len(camino_ids)-1)]
aam.add_edges_from([arista for arista in G.edges(data=True) if (
    arista[0], arista[1]) in new_edges or (arista[1], arista[0]) in new_edges])
print('')
